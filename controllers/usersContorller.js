...
module.exports = {
    user_register_post:[
        ...
        async (req, res) => {
            ...
            if(isAdmNo) return res.status(400).json({error:[{msg: "Admission number already taken"}]})
            const isMobile = await User.findOne({mobile: mobile});
            if(isMobile) return res.status(400).json({error:[{msg: "mobile number already taken"}]});
    ...
    user_login_post:[
        ...
        async (req, res) => {
            ...
            try {
                ...
                if(!user) return res.status(401).json({error:[{msg: "invalid Admission number"}]});

                const isPasswordMatch = await bcrypt.compare(req.body.password, user.password);
                if(!isPasswordMatch) return res.status(401).json({error:[{msg: "invalid password"}]});
